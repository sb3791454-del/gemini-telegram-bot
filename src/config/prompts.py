...
• *مارکیٹ الرٹس (Price & Setup Alerts):* خودکار پرائس، RSI اور سیٹ اپ نوٹیفکیشنز (`/alert`)
...
• `/alert <symbol> <condition>` - پرائس، RSI یا سیٹ اپ الرٹ لگائیں
• `/alerts` - ایکٹو الرٹس کی فہرست دیکھیں
...
🔔 *مارکیٹ الرٹس اور مانیٹرنگ (Phase 9):*
• `/alert <symbol> > <price>` — قیمت اوپر جانے پر الرٹ (مثلاً: `/alert BTCUSDT > 72000`)
• `/alert <symbol> < <price>` — قیمت نیچے آنے پر الرٹ (مثلاً: `/alert SOLUSDT < 140`)
• `/alert <symbol> rsi > <val>` — اوور باٹ RSI الرٹ (مثلاً: `/alert ETHUSDT rsi > 70`)
• `/alert <symbol> setup` — جب ٹریڈ سیٹ اپ ایکشن ایبل ہو جائے (`SETUP_READY`)
• `/alert <symbol> pullback` — پل بیک انٹری زون میں آنے پر الرٹ
• `/alerts` — تمام ایکٹو اور پاز الرٹس کی فہرست
• `/unalert <id>` — کوئی مخصوص الرٹ ڈیلیٹ کریں (مثلاً: `/unalert 1`)
• `/pausealert <id>` / `/resumealert <id>` — الرٹ کو عارضی طور پر روکیں یا دوبارہ شروع کریں
